...
public class PaymentService {
    ...
    public PaymentResponseDto verifyAndSavePayment(PaymentRequestDto dto) {
        ...
        Payment payment = PaymentMapper.toEntity(dto.getPaymentKey(), dto.getAmount(), order, dto.getPaymentStatus());
        if(dto.getPaymentStatus() == Payment.PaymentStatus.SUCCESS) {
            payment.markSuccess(LocalDateTime.now());
        } else if(dto.getPaymentStatus() == Payment.PaymentStatus.FAIL) {
            payment.markFail(LocalDateTime.now());
        }
